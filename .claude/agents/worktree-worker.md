...
2. `adachi/develop` から指定されたブランチ名で新ブランチを作成: `git checkout -b {branch} adachi/develop`
...
9. `gh pr create --base adachi/develop` でPRを作成する
10. 複数タスクがある場合は、次のタスクへ進む（`git checkout -b {next-branch} adachi/develop`）
...
- `adachi/develop` ブランチを基点にブランチを作成する
- タスクの範囲外の変更はしない
